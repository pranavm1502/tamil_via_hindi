...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tamil Setu Roadmap')),
      body: GridView.builder(
        padding: const EdgeInsets.all(16),
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: 2, crossAxisSpacing: 16, mainAxisSpacing: 16,
        ),
        itemCount: 5, // Match your curriculum levels
        itemBuilder: (context, index) {
          int levelId = index + 1;
          bool locked = progress.isLevelLocked(levelId);

          return GestureDetector(
            onTap: locked ? null : () => Navigator.push(context, 
                MaterialPageRoute(builder: (ctx) => LessonScreen(levelId: levelId))),
            child: Card(
              color: locked ? Colors.grey[300] : Colors.white,
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(locked ? Icons.lock : Icons.play_circle, size: 40),
                  Text("Level $levelId"),
                ],
              ),
            ),
